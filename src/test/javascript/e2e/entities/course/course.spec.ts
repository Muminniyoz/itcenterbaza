...
describe('Course e2e test', () => {
  ...
  it('should load Courses', async () => {
    ...
    expect(await courseComponentsPage.getTitle()).to.eq('itcenterbazaApp.course.home.title');
  ...
  it('should load create Course page', async () => {
    ...
    expect(await courseUpdatePage.getPageTitle()).to.eq('itcenterbazaApp.course.home.createOrEditLabel');
  ...
  it('should create and save Courses', async () => {
    ...
    await promise.all([
      ...
      courseUpdatePage.setStartDateInput('2000-12-31'),
      courseUpdatePage.statusSelectLastOption(),
      courseUpdatePage.setDurationInput('5'),
      courseUpdatePage.setPlanFileInput(absolutePath),
      courseUpdatePage.teacherSelectLastOption(),
      courseUpdatePage.centerSelectLastOption(),
      courseUpdatePage.skillSelectLastOption(),
    ]);
    ...
    expect(await courseUpdatePage.getStartDateInput()).to.eq('2000-12-31', 'Expected startDate value to be equals to 2000-12-31');
    expect(await courseUpdatePage.getDurationInput()).to.eq('5', 'Expected duration value to be equals to 5');
    expect(await courseUpdatePage.getPlanFileInput()).to.endsWith(
      fileNameToUpload,
  ...
  it('should delete last Course', async () => {
    ...
    expect(await courseDeleteDialog.getDialogTitle()).to.eq('itcenterbazaApp.course.delete.question');
